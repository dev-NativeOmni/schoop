...
class FinanceAnalyticsController extends Controller
{
    public function __construct(
        ...
        private readonly FinanceAnalyticsService $service,
    ...
    public function index(SchoolAnalyticsFilterRequest $request)
    {
        ...
        abort_unless($schoolId, 403, 'Akses analitik keuangan memerlukan tenant context.');
        ...
        $this->logger->log($request, 'finance_analytics', 'view', (int) $schoolId);

        $data = $this->service->data((int) $schoolId, $period['date_from'], $period['date_until']);

        return view('analytics.finance.dashboard', array_merge($data, [
            'schoolId' => $schoolId,
