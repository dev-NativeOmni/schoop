...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-2 text-sm text-slate-500">
        ...
        <span>/</span>
        <span class="text-slate-900 dark:text-white font-semibold">Tambah Langganan Sekolah</span>
    </div>

    <!-- Header -->
    <div>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Tambah Langganan Sekolah</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400">Hubungkan sekolah ke paket langganan secara manual.</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-slate-900 shadow-xl rounded-2xl border border-slate-100 dark:border-slate-800 p-6 md:p-8">
        <form action="{{ route('billing.school-subscriptions.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="subscription_plan_id" name="subscription_plan_id" required class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition">
                        ...
                        @foreach($plans as $plan)
                            <option value="{{ $plan->id }}" {{ old('subscription_plan_id') == $plan->id ? 'selected' : '' }}>{{ $plan->name }} (IDR {{ number_format($plan->monthly_price, 0, ',', '.') }}/bln)</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" id="starts_at" name="starts_at" value="{{ old('starts_at', date('Y-m-d')) }}" class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition">
                    @error('starts_at')
                        <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="trial_ends_at" name="trial_ends_at" value="{{ old('trial_ends_at') }}" class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition">
                    @error('trial_ends_at')
                        <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="current_period_starts_at" name="current_period_starts_at" value="{{ old('current_period_starts_at', date('Y-m-d')) }}" class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition">
                ...
                <div>
                    ...
                    <input type="date" id="current_period_ends_at" name="current_period_ends_at" value="{{ old('current_period_ends_at', date('Y-m-d', strtotime('+1 month'))) }}" class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition">
            ...
            <div>
                ...
                <textarea id="metadata_raw" name="metadata_raw" rows="3" placeholder='e.g. {"payment_reference": "manual_inv_123"}' class="w-full font-mono text-xs rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition">{{ old('metadata_raw') }}</textarea>
                @error('metadata')
                    <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
