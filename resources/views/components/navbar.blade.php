...
<nav x-data="{ open: false, openDropdown: null }" 
    ...
    <div x-show="open" x-transition class="lg:hidden border-t border-slate-750 bg-[#1F2937]/95 backdrop-blur-md px-4 sm:px-6 py-4 space-y-2 text-white" style="display: none;" @click.away="open = false">
        <a href="{{ route('dashboard') }}" class="block rounded-xl px-3 py-2.5 text-sm font-semibold transition {{ request()->routeIs('dashboard*') ? 'bg-slate-800 text-[#A3E635]' : 'hover:bg-slate-800 text-slate-300' }}">Dashboard</a>
        <a href="{{ route('quran.mushaf') }}" class="block rounded-xl px-3 py-2.5 text-sm font-semibold transition {{ request()->routeIs('quran*') ? 'bg-slate-800 text-[#A3E635]' : 'hover:bg-slate-800 text-slate-300' }}">Al-Qur'an</a>
