...
<div class="space-y-8">
    ...
    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <!-- Stat Cards -->
        <div class="group rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md hover:scale-[1.01]">
            <div class="flex items-center justify-between">
                <span class="text-sm font-bold text-slate-500">Total Siswa</span>
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600 transition-colors group-hover:bg-indigo-100">
            ...
            <div class="mt-4">
                ...
                <span class="block text-xs text-slate-400 mt-1">Siswa aktif terdaftar</span>
            </div>
        </div>

        <div class="group rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md hover:scale-[1.01]">
            <div class="flex items-center justify-between">
                <span class="text-sm font-bold text-slate-500">Kelas Terdaftar</span>
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600 transition-colors group-hover:bg-emerald-100">
            ...
            <div class="mt-4">
                <span class="text-3xl font-black text-slate-900">{{ $stats['classrooms_count'] }}</span>
                <span class="block text-xs text-slate-400 mt-1">Kelas aktif terdaftar</span>
            </div>
        </div>

        <div class="group rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md hover:scale-[1.01]">
            <div class="flex items-center justify-between">
                <span class="text-sm font-bold text-slate-500">User Memberships</span>
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-50 text-amber-600 transition-colors group-hover:bg-amber-100">
                    <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                </div>
            </div>
            <div class="mt-4">
                <span class="text-3xl font-black text-slate-900">{{ $stats['memberships_count'] }}</span>
                <span class="block text-xs text-slate-400 mt-1">Staf & Guru terhubung</span>
            </div>
        </div>

        <div class="group rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md hover:scale-[1.01]">
            <div class="flex items-center justify-between">
                <span class="text-sm font-bold text-slate-500">Module Aktif</span>
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-lime-50 text-lime-700 transition-colors group-hover:bg-lime-100">
            ...
            <div class="mt-4">
                <span class="text-3xl font-black text-slate-900">{{ $stats['modules_enabled_count'] }}</span>
                <span class="block text-xs text-slate-400 mt-1">Module diaktifkan</span>
            </div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-5 bg-slate-50/50">
                    <h2 class="text-lg font-black text-slate-900">Module Configuration</h2>
                ...
                <div class="p-6">
                    <div class="grid gap-3 sm:grid-cols-2">
                        @forelse($modules as $module)
                            <div class="flex items-center justify-between rounded-2xl border border-slate-100 p-4 transition-all hover:border-slate-200 hover:bg-slate-50/40">
                                <span class="text-sm font-bold text-slate-800">{{ $module->module_name }}</span>
                        ...
                        @empty
                            <p class="text-sm text-slate-500 col-span-2 text-center py-4">Belum ada module terdaftar.</p>
                        @endforelse
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-5 bg-slate-50/50">
                    <h2 class="text-lg font-black text-slate-900">Tenant Settings</h2>
                    ...
                    <div class="space-y-4">
                        @forelse($settings as $setting)
                            <div class="flex items-center justify-between border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                                <div>
                                    <span class="block text-sm font-bold text-slate-800">{{ $setting->setting_key }}</span>
                                    <span class="block text-xs text-slate-400 mt-0.5">{{ $setting->description ?? 'Tidak ada deskripsi' }}</span>
                                </div>
                                <span class="text-sm font-bold text-slate-600">{{ Str::limit($setting->setting_value, 40) }}</span>
                            </div>
                        @empty
                            <p class="text-sm text-slate-500 text-center py-4">Belum ada settings yang diset.</p>
                        @endforelse
                    </div>
                </div>
            </div>
                ...
                <div class="p-6 flex-1 overflow-y-auto">
                    <div class="flow-root">
                        <ul role="list" class="-mb-8">
                                ...
                                <li>
                                    <div class="relative pb-8">
                                        ...
                                        <div class="relative flex space-x-3">
                                            <div>
                                                <span class="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 ring-8 ring-white text-slate-600">
                                            ...
                                            <div class="flex-1 min-w-0 pt-1.5">
                                                ...
                                                <p class="text-xs text-slate-400 mt-1">
                                                    Oleh: <strong class="text-slate-600">{{ $log->user?->name ?? 'System' }}</strong>
                                                </p>
                                                <time class="block text-[10px] text-slate-400 mt-0.5">
                            ...
                            @empty
                                <p class="text-sm text-slate-500 text-center py-4">Belum ada log aktivitas.</p>
                            @endforelse
                        </ul>
                    </div>
                </div>
