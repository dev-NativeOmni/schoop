...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-black text-slate-900">Tambah Anggota Baru</h1>
            <p class="text-xs text-slate-400 mt-0.5">Hubungkan user yang terdaftar ke sekolah <strong class="text-slate-600">{{ $school->name }}</strong></p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('tenancy.memberships.store') }}" method="POST" class="space-y-6">
            @csrf
            <input type="hidden" name="school_id" value="{{ $school->id }}">

            <!-- User Select -->
            <div class="space-y-2">
                <label for="user_id" class="text-sm font-bold text-slate-700">Pilih User</label>
                <select id="user_id" name="user_id" required class="block w-full rounded-2xl border border-slate-200 px-4 py-3 text-slate-800 focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div class="space-y-2">
                ...
                <select id="membership_status" name="membership_status" required class="block w-full rounded-2xl border border-slate-200 px-4 py-3 text-slate-800 focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <option value="inactive" {{ old('membership_status') == 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                </select>
            ...
            <div class="space-y-2">
                ...
                <input type="date" id="joined_at" name="joined_at" value="{{ old('joined_at', date('Y-m-d')) }}" required class="block w-full rounded-2xl border border-slate-200 px-4 py-3 text-slate-800 focus:border-indigo-500 focus:ring-indigo-500">
                @error('joined_at')
                    <p class="text-xs font-semibold text-red-650 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center space-x-3">
                <input type="checkbox" id="is_default" name="is_default" value="1" {{ old('is_default') ? 'checked' : '' }} class="h-4.5 w-4.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
