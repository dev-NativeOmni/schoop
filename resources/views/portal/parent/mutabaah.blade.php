...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200/80 pb-5 dark:border-slate-800">
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white flex items-center gap-3">
                ...
                <span>Mutabaah Anak</span>
            </h1>
            <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Pantau perkembangan ibadah dan aktivitas harian anak Anda secara real-time.</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <form method="GET" action="{{ route('portal.parent.mutabaah', $student ?? '') }}" class="grid gap-4 sm:grid-cols-4 items-end">
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Anak</label>
    ...
    @if(isset($student))
        ...
        <div class="grid gap-6 sm:grid-cols-3">
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 transition hover:shadow-md">
                ...
                <p class="mt-1 text-xs text-slate-400">Total pencatatan periode ini</p>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 transition hover:shadow-md">
                ...
                <p class="mt-1 text-xs text-slate-400">Aktivitas terlaksana dengan baik</p>
            </div>

            <!-- Completion Rate -->
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 transition hover:shadow-md">
                <p class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Rasio Capaian</p>
    ...
    @else
        <div class="rounded-2xl border border-dashed border-slate-300 p-12 text-center dark:border-slate-700">
            ...
            <h3 class="mt-4 text-sm font-bold text-slate-900 dark:text-white">Belum ada data anak</h3>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Hubungi admin sekolah untuk menghubungkan data anak ke akun Anda.</p>
        </div>
    @endif
