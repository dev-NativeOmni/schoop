            ...
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="bg-slate-50/40 dark:bg-slate-850/40 border-b border-slate-200/80 dark:border-slate-800">
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Password Plain</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse ($users as $user)
                        <tr class="hover:bg-slate-50/40 dark:hover:bg-slate-850/10 transition-all">
                            ...
                            <td class="px-6 py-4">
                                @if($user->decrypted_password !== '-')
                                    <div class="flex items-center space-x-2">
                                        <span class="font-mono text-xs select-all text-slate-700 dark:text-slate-300 font-bold" style="-webkit-text-security: disc; text-security: disc;">{{ $user->decrypted_password }}</span>
                                        <button type="button" onclick="togglePasswordVisibility(this)" class="text-slate-400 hover:text-[#A3E635] transition focus:outline-none" title="Tampilkan Password">
                                            <svg class="h-4.5 w-4.5 eye-open-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-lg bg-slate-100 dark:bg-slate-800 px-2 py-1 text-xs font-bold text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 mb-1">
                                    {{ $user->role?->name === 'super_admin' ? 'Super Admin' : ($user->role?->name === 'admin' ? 'Admin Sekolah' : ($user->role?->name === 'principal' ? 'Kepala Sekolah' : ($user->role?->name === 'teacher' ? 'Guru' : ($user->role?->name === 'parent' ? 'Orang Tua' : ($user->role?->name === 'student' ? 'Santri' : $user->role?->name))))) }}
                                </span>
...
<script>
    function togglePasswordVisibility(btn) {
        ...
        if (textSpan.style.webkitTextSecurity === 'none' || textSpan.style.textSecurity === 'none') {
            textSpan.style.webkitTextSecurity = 'disc';
            textSpan.style.textSecurity = 'disc';
            btn.innerHTML = `
            ...
            textSpan.style.webkitTextSecurity = 'none';
            textSpan.style.textSecurity = 'none';
            btn.innerHTML = `
                <svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
