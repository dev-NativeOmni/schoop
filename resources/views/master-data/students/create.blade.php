...
@section('content')
    <div class="mb-6">
        <h2 class="text-2xl font-bold">Tambah Santri</h2>
        <p class="text-sm text-slate-500">Buat data santri baru.</p>
    </div>

    <div class="max-w-xl rounded-2xl bg-white p-6 shadow-sm">
        <form method="POST" action="{{ route('master-data.students.store') }}" class="space-y-4" id="studentForm">
            @csrf
            ...
            <div class="border-t border-slate-100 pt-4 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="full_name" value="{{ old('full_name') }}" class="w-full rounded-lg border border-slate-300 px-4 py-2" required>
                    @error('full_name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold">Nama Panggilan</label>
                    <input type="text" name="nickname" value="{{ old('nickname') }}" class="w-full rounded-lg border border-slate-300 px-4 py-2">
                ...
                <div>
                    ...
                    <input type="text" name="student_number" value="{{ old('student_number') }}" class="w-full rounded-lg border border-slate-300 px-4 py-2">
                ...
                <div>
                    ...
                    <input type="text" name="nisn" value="{{ old('nisn') }}" class="w-full rounded-lg border border-slate-300 px-4 py-2">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="birth_place" value="{{ old('birth_place') }}" class="w-full rounded-lg border border-slate-300 px-4 py-2">
                    ...
                    <div>
                        ...
                        <input type="date" name="birth_date" value="{{ old('birth_date') }}" class="w-full rounded-lg border border-slate-300 px-4 py-2">
                ...
                <div>
                    ...
                    <input type="text" name="program_type" value="{{ old('program_type', 'tahfizh') }}" class="w-full rounded-lg border border-slate-300 px-4 py-2">
                ...
                <div>
                    ...
                    <select name="parent_profile_ids[]" class="w-full rounded-lg border border-slate-300 px-4 py-2 h-24" multiple>
                        @foreach ($parents as $parent)
                            <option value="{{ $parent->id }}" @selected(in_array($parent->id, old('parent_profile_ids', [])))>
            ...
            <div class="border-t border-slate-100 pt-4 space-y-4">
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="create_login_account" value="1" id="create_login_account" @checked(old('create_login_account')) onchange="toggleAccountFields(this)">
                    <label for="create_login_account" class="text-sm font-bold text-slate-700">Buat Akun Login Santri</label>
                </div>

                <div class="space-y-4 {{ old('create_login_account') ? '' : 'hidden' }}" id="accountFields">
                    <div>
                        <label class="mb-2 block text-sm font-semibold">Username (Unique)</label>
                        <input type="text" name="username" value="{{ old('username') }}" class="w-full rounded-lg border border-slate-300 px-4 py-2">
                    ...
                    <div>
                        ...
                        <input type="password" name="password" class="w-full rounded-lg border border-slate-300 px-4 py-2" placeholder="Min 8 karakter">
