...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200/80 pb-5 dark:border-slate-800 gap-4">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h1 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Buat Session Presensi</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">Buat jadwal pencatatan kehadiran baru untuk santri.</p>
            </div>
        </div>
    </div>
    ...
    {{-- Form --}}
    <form action="{{ route('attendance.sessions.store') }}" method="POST" class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 space-y-6">
        @csrf
