...
@section('content')
    ...
    <div class="mb-6 rounded-2xl bg-white p-4 shadow-sm border border-slate-100">
        <form method="GET" action="{{ route('reports.tahfizh.dashboard') }}"
            ...
            <div>
                <label for="date_from" class="mb-1 block text-xs font-semibold text-slate-600">Dari</label>
                <input type="date" name="date_from" id="date_from" value="{{ request('date_from', $dateFrom->format('Y-m-d')) }}"
                       class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
            </div>

            <div>
                <label for="date_until" class="mb-1 block text-xs font-semibold text-slate-600">Sampai</label>
    ...
    <div class="grid gap-4 md:grid-cols-4">
        <div class="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
            <div class="text-sm text-slate-500 font-semibold">Total Santri</div>
            <div class="mt-2 text-3xl font-extrabold text-slate-900">{{ $summary['total_students'] }}</div>
        </div>

        <div class="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
            <div class="text-sm text-slate-500 font-semibold">Total Setoran</div>
            <div class="mt-2 text-3xl font-extrabold text-slate-900">{{ $summary['total_records'] }}</div>
        </div>

        <div class="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
            <div class="text-sm text-slate-500 font-semibold">Total Baris</div>
            <div class="mt-2 text-3xl font-extrabold text-indigo-700">+{{ $summary['total_lines'] }}</div>
        </div>

        <div class="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
            <div class="text-sm text-slate-500 font-semibold">Akumulasi Hutang</div>
            <div class="mt-2 text-3xl font-extrabold text-red-600">{{ $summary['total_cumulative_debt_lines'] }} baris</div>
        </div>
    </div>
    ...
    <div class="mt-6 grid gap-4 md:grid-cols-4">
        <div class="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
            <div class="text-sm text-slate-500 font-semibold">Tercapai</div>
            <div class="mt-2 text-2xl font-bold text-green-700">{{ $summary['met_count'] }}</div>
        </div>

        <div class="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
            <div class="text-sm text-slate-500 font-semibold">Kurang</div>
            <div class="mt-2 text-2xl font-bold text-red-600">{{ $summary['behind_count'] }}</div>
        </div>

        <div class="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
            <div class="text-sm text-slate-500 font-semibold">Lebih</div>
            <div class="mt-2 text-2xl font-bold text-blue-600">{{ $summary['ahead_count'] }}</div>
        </div>

        <div class="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
            <div class="text-sm text-slate-500 font-semibold">Belum Ada Target</div>
            <div class="mt-2 text-2xl font-bold text-slate-500">{{ $summary['no_target_count'] }}</div>
        </div>
    </div>
    ...
    <div class="mt-8 grid gap-6 md:grid-cols-2">
        ...
        <div class="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
            <h3 class="mb-4 text-lg font-bold text-slate-900">Aktivitas Guru</h3>
                    ...
                    <tbody>
                        @forelse ($summary['teacher_activity'] as $activity)
                            <tr class="border-b hover:bg-slate-50">
                                ...
                                <td class="py-2.5 px-3 text-indigo-700 font-semibold">+{{ $activity->total_lines }}</td>
                            </tr>
                        @empty
                            <tr>
        ...
        <!-- At Risk Students -->
        <div class="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
            <h3 class="mb-4 text-lg font-bold text-slate-900">Santri Perlu Perhatian</h3>
                ...
                <table class="w-full text-left text-sm border-collapse">
                    ...
                    <tbody>
                        @forelse ($summary['at_risk_students'] as $debt)
                            <tr class="border-b hover:bg-slate-50">
                                <td class="py-2.5 px-3 font-semibold text-slate-900">{{ $debt->student?->full_name ?? '-' }}</td>
                                <td class="py-2.5 px-3">{{ $debt->student?->classRoom?->name ?? '-' }}</td>
                                <td class="py-2.5 px-3 font-bold text-red-600">{{ $debt->cumulative_debt_lines }} baris</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="py-4 px-3 text-center text-slate-500">
