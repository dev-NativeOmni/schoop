...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200/80 pb-5 dark:border-slate-800 gap-4">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Laporan Bulanan Tahfizh</h2>
    ...
    <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <form method="GET" action="{{ route('reports.tahfizh.monthly.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 items-end">
                <div>
                    <label for="month" class="mb-1.5 block text-xs font-bold text-slate-500 dark:text-slate-455 uppercase tracking-wider">Bulan</label>
                    <input type="month" name="month" id="month" value="{{ request('month', $month) }}" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500">
                </div>
                ...
                <div>
                    <label for="status" class="mb-1.5 block text-xs font-bold text-slate-500 dark:text-slate-455 uppercase tracking-wider">Status</label>
                    <select name="status" id="status" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500">
                        <option value="">Semua Status</option>
                        @foreach ($statuses as $stat)
                            <option value="{{ $stat }}" @selected(request('status') === $stat)>
            ...
            <div class="flex items-center justify-end space-x-2 pt-2 border-t border-slate-100 dark:border-slate-800/60">
                <a href="{{ route('reports.tahfizh.monthly.index') }}" class="inline-flex items-center space-x-1.5 rounded-full border border-slate-200 px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 dark:border-slate-800 dark:text-slate-400 dark:hover:bg-slate-800 transition-all focus:outline-none">
                    Reset Filter
                </a>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse ($rows as $row)
                        <tr class="hover:bg-slate-50/40 dark:hover:bg-slate-850/10 transition-all">
                            ...
                            <td class="px-6 py-4 text-center">
                                <a href="{{ route('reports.tahfizh.monthly.show', ['student' => $row['student']->id, 'month' => $month]) }}" class="inline-flex h-8 px-3.5 items-center justify-center rounded-full border border-slate-200 text-slate-600 hover:bg-slate-50 text-xs font-bold dark:border-slate-800 dark:text-slate-400 dark:hover:bg-slate-800 transition-all">
                                    Detail
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="10" class="px-6 py-8 text-center text-sm text-slate-450 dark:text-slate-500">Belum ada data laporan bulanan tahfizh.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
