...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center space-x-3 border-b border-slate-200/80 pb-5 dark:border-slate-800">
        ...
        <div>
            <h2 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Edit Target Tahfizh</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400">Perbarui konfigurasi target hafalan tahfizh.</p>
        </div>
    </div>

    {{-- Form Card --}}
    <div class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <form method="POST" action="{{ route('tahfizh.targets.update', $target) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    ...
                    <select name="school_id" id="school_id" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none @error('school_id') border-red-500 @enderror" required>
                        ...
                        @foreach ($schools as $school)
                            <option value="{{ $school->id }}" {{ old('school_id', $target->school_id) == $school->id ? 'selected' : '' }}>
                                {{ $school->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $target->name) }}" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none @error('name') border-red-500 @enderror" required>
                    @error('name')
                        <p class="mt-1.5 text-xs text-rose-600 dark:text-rose-450">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="class_room_id" id="class_room_id" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none @error('class_room_id') border-red-500 @enderror">
                        ...
                        @foreach ($classRooms as $classRoom)
                            <option value="{{ $classRoom->id }}" {{ old('class_room_id', $target->class_room_id) == $classRoom->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="program_type" id="program_type" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none @error('program_type') border-red-500 @enderror">
                        ...
                        <option value="reguler" {{ old('program_type', $target->program_type) == 'reguler' ? 'selected' : '' }}>Reguler</option>
                        <option value="takhassus" {{ old('program_type', $target->program_type) == 'takhassus' ? 'selected' : '' }}>Takhassus</option>
                    </select>
                    @error('program_type')
                        <p class="mt-1.5 text-xs text-rose-600 dark:text-rose-450">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="daily_target_lines" id="daily_target_lines" min="0" max="300" value="{{ old('daily_target_lines', $target->daily_target_lines) }}" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none @error('daily_target_lines') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <input type="number" name="weekly_target_lines" id="weekly_target_lines" min="0" max="1500" value="{{ old('weekly_target_lines', $target->weekly_target_lines) }}" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none @error('weekly_target_lines') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <input type="number" name="monthly_target_lines" id="monthly_target_lines" min="0" max="6000" value="{{ old('monthly_target_lines', $target->monthly_target_lines) }}" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none @error('monthly_target_lines') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <input type="date" name="effective_from" id="effective_from" value="{{ old('effective_from', $target->effective_from ? $target->effective_from->format('Y-m-d') : '') }}" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none @error('effective_from') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="date" name="effective_until" id="effective_until" value="{{ old('effective_until', $target->effective_until ? $target->effective_until->format('Y-m-d') : '') }}" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none @error('effective_until') border-red-500 @enderror">
            ...
            <div class="flex items-center p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-150 dark:border-slate-850">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $target->is_active) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 dark:bg-slate-950 dark:border-slate-800">
