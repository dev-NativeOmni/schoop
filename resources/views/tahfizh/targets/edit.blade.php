...
@section('content')
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold mt-2">Edit Target Tahfizh</h2>
        <p class="text-sm text-slate-500">Perbarui konfigurasi target hafalan.</p>
    </div>
        ...
        <form method="POST" action="{{ route('tahfizh.targets.update', $target) }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $target->name) }}" class="mt-1 w-full rounded-lg border border-slate-200 p-2.5 text-sm @error('name') border-red-500 @enderror" required>
                    @error('name')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="daily_target_lines" id="daily_target_lines" min="0" max="300" value="{{ old('daily_target_lines', $target->daily_target_lines) }}" class="mt-1 w-full rounded-lg border border-slate-200 p-2.5 text-sm @error('daily_target_lines') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <input type="number" name="weekly_target_lines" id="weekly_target_lines" min="0" max="1500" value="{{ old('weekly_target_lines', $target->weekly_target_lines) }}" class="mt-1 w-full rounded-lg border border-slate-200 p-2.5 text-sm @error('weekly_target_lines') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <input type="number" name="monthly_target_lines" id="monthly_target_lines" min="0" max="6000" value="{{ old('monthly_target_lines', $target->monthly_target_lines) }}" class="mt-1 w-full rounded-lg border border-slate-200 p-2.5 text-sm @error('monthly_target_lines') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <input type="date" name="effective_from" id="effective_from" value="{{ old('effective_from', $target->effective_from ? $target->effective_from->format('Y-m-d') : '') }}" class="mt-1 w-full rounded-lg border border-slate-200 p-2.5 text-sm @error('effective_from') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="date" name="effective_until" id="effective_until" value="{{ old('effective_until', $target->effective_until ? $target->effective_until->format('Y-m-d') : '') }}" class="mt-1 w-full rounded-lg border border-slate-200 p-2.5 text-sm @error('effective_until') border-red-500 @enderror">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $target->is_active) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900">
