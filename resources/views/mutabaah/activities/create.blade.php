...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-4">
        ...
        <h1 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Tambah Aktivitas Mutabaah</h1>
    </div>

    {{-- Form Card --}}
    <div class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <form method="POST" action="{{ route('mutabaah.activities.store') }}" id="form-activity" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          ...
                          placeholder="Berikan keterangan singkat atau instruksi pengerjaan aktivitas..."></textarea>
                @error('description')
                    <p class="mt-1.5 text-xs font-semibold text-rose-600 dark:text-rose-450">{{ $message }}</p>
                @enderror
            </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    @foreach(['checklist' => ['icon' => 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', 'label' => 'Checklist', 'desc' => 'Ya / Tidak'],
                        ...
                        <label class="relative block rounded-xl border border-slate-200 bg-white p-3.5 cursor-pointer hover:bg-slate-50/50 focus-within:ring-2 focus-within:ring-indigo-500/20 dark:border-slate-800 dark:bg-slate-950 dark:hover:bg-slate-900 transition">
                            <input type="radio" name="input_type" value="{{ $type }}" class="sr-only peer" @checked(old('input_type', 'checklist') === $type) required>
            ...
            <div class="border-t border-slate-100 dark:border-slate-800 pt-5">
                ...
                <div class="space-y-3">
                    <label class="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" name="is_required" id="is_required" value="1" @checked(old('is_required'))
                    ...
                    <label class="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" name="allow_teacher_input" id="allow_teacher_input" value="1" @checked(old('allow_teacher_input', true))
                               class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500/30 h-4.5 w-4.5">
                        <span class="text-sm font-semibold text-slate-700 dark:text-slate-350">Guru boleh input</span>
                    </label>
                    <label class="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" name="allow_parent_input" id="allow_parent_input" value="1" @checked(old('allow_parent_input'))
                               class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500/30 h-4.5 w-4.5">
                        <span class="text-sm font-semibold text-slate-700 dark:text-slate-350">Orang tua boleh input</span>
                    </label>
                    <label class="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" name="allow_student_input" id="allow_student_input" value="1" @checked(old('allow_student_input'))
                               class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500/30 h-4.5 w-4.5">
